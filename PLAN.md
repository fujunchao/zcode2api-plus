...
响应读取预算分开管理：HTTP 错误体仍最多读取 64 KiB；正常非流式 JSON 使用
16 MiB 的独立预算，超限明确返回 502 `upstream_response_too_large`，不能静默裁切后
返回 200。SSE 按块转发，不受该总大小限制；三个同步接口共用这套规则。
...
- 固定头：`anthropic-version: 2023-06-01`、`User-Agent: ZCode/<ver>`、`X-ZCode-App-Version: <ver>`
  （两处 `<ver>` 均取自 `config.ZcodeClientVersion`，当前 3.14.3；勿在文档/代码里写死版本号）、
...
同时命中——只探测通了额度就复活账号，一次轮询（15–60s）就把刚升上去的封禁悄悄抹掉。
保护由 `quota.apply` 在统一的持锁回写边界执行：若当前为 `invalid` 且
`last_error_kind == risk_control`，所有额度写入都保留状态、冷却字段与原始错误证据，
只更新检查时间、套餐及额度快照。覆盖查询失败、405 幂等、空额度和额度耗尽路径，
防止「先覆盖错误归类，下一次成功便解封」；重开数据库后同样有效，不新增账号 JSON 字段。
其它成因的 `invalid`（如凭据失效）保持「额度通了即视为恢复」。这条守卫**不让计费路径驱动阶梯**
