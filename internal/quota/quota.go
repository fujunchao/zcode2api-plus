...
func (s *Service) handleBillingResponse(acc *model.Account, checkedAt float64, resp *http.Response) map[string]any {
	...
	//
	// 键必须是可哈希的字符串：上游把 entitlement_id 回成数组或对象时，直接拿它
	// 当 map[any] 的键会 panic（hash of unhashable type），而本函数跑在无 recover
	// 的 goroutine 里，会终止整个进程。
...
func (s *Service) RefreshAccounts(accounts []*model.Account) map[string]any {
	...
	for _, acc := range accounts {
		...
		go func(acc *model.Account) {
			...
			// 不在其保护范围内（对齐 Python gather 的 return_exceptions=True）。
			defer func() {
				if r := recover(); r != nil {
...
// refreshOnce 执行一轮额度刷新，并把任何 panic 隔离在本轮之内。
//
// 后台循环出错应当记日志后继续下一轮，而不是让监控循环终止——那样额度再也不刷新，
// 且没有任何提示（对齐 Python _loop 里的 `except Exception: logs.err(...)`）。
